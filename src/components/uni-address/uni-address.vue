<template>
  <view v-if="showPopup" class="simple-address" @touchmove.stop.prevent="clear">
    <!-- 遮罩层 -->
    <view
      v-if="maskClick"
...
export default {
  ...
  methods: {
    ...
    queryIndex (params = [], type = 'value') {
      ...
      let cityIndex = cityData[provinceIndex].findIndex(
        res => res[type] == params[1]
      )
